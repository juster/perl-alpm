...
ALPM - XS module for libalpm (ArchLinux Package Manager)

=head1 VERSION

0.04 -- compatible with pacman 3.3.0's alpm
...
=head1 SYNOPSIS
  ...
  print map { sprintf "%10s: %s %s\n", $_->db->get_name, $_->name, $_->version }
            ALPM->search('perl');
  ...
  #$trans->prepare; # unnecessary, commit calls prepare if needed
  $trans->commit;
  undef $trans;     # to force releasing, now!

=head1 DESCRIPTION

This version of ALPM works with the libalpm included with version 3.30
of pacman.
...
=item 1. The path to a pacman.conf configuration file.  Example:

  use ALPM qw( /etc/pacman.conf );

=item 2. A hash of options to use for ALPM.  Example:
...
It is important to set options as soon as possible.
...
You can also use the method C<register> which is a shorter alias for
C<register_db>.
...
This is a shorter alias for the method C<repodb>.
...
This is a shorter alias for the method C<databases>.
...
=item type - A string, valid types are:
...
representing the event.  Events are explained in greater
detail in the L<ALPM::Transaction> document.

=item conv - A coderef, used as a conversation callback.

=item progress - A coderef, used a progress callback.
