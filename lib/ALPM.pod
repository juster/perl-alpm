...
=head2 get_dbs
  ...
  Usage   : my %dbs = ALPM->get_dbs;
  Params  : None
  Returns : A hash of 'database name' => 'database object'
            for each database that is registered.
...
=item flags - An arrayref of strings, valid flags are:
