...
=head1 SYNOPSIS
  ...
  our $alpm; # uses "our" package variable, not "my"
  
  ## Querying databases & packages
  my $localdb = $alpm->get_localdb;
  my $pkg = $localdb->find('perl') or die 'wtfbbq';
  printf "%s %s %s %d\n", $pkg->name, $pkg->version, $pkg->arch, $pkg->size;
  
  my $extradb = $alpm->register('extra', 'ftp://ftp.archlinux.org/extra/os/i686' )
  	or die $alpm->errstr;
  $extradb->update or die $alpm->errstr;
...
If the download fails. Check L</errstr>.
...
Signature level hashref. See L</Signature Level>.
...
=head2 register
  ...
  $DB | undef = $OBJ->register($NAME, $URL)
...
The URL provides the base directory where the database files are
publicly available.
...
Returns 1 on success or undef on error. Check L</errstr> on error.

=head2 set_pkg_reason

  1 | undef = $OBJ->set_pkg_reason($PKG, $REASON)

Set a local package installation reason. Returns 1 on success, undef on error.
Check L</errstr> on error. This would make more sense as an L<ALPM::DB::Local>
object method, but this matches the I<alpm_db_set_pkgreason> C function.

=over 4

=item $PKG

An L<ALPM::Package> object. I assume this must be a package retrieved
from the localdb.

=item $REASON

Must be either C<"explicit"> or C<"implicit"/"depend">.
...
=item B<usedelta>+ - should you use deltas? not really sure, never tried it...

=item B<checkspace> - check for available diskspace

=item B<defsiglvl> - the "default" (is there another?) signature level. See L</Signature Level>. This name was shortened from I<alpm_option_set_default_signature_level>. You're welcome.
...
=item B<syncdbs>* - a list of sync databases as ALPM::DB objects
...
<=, >, <, or ? if an internal error occurred and the dependency is unknown.
...
Signature levels describe the level of security which is required by packages or
by databases. These determine what degree of signature checking is performed.
Bear with me as I'm not sure I understand them myself.

A "siglvl" can either be a string or a hashref. Strings must either be C<"default"> or
C<"never">. I'm not sure what C<"default"> signifies. A siglvl hashref contains a
I<pkg> key and a I<db> key. If those keys are missing, it is the same as those keys
having the value of C<"never">.
...
Trusts signatures regardless of whose signature it is. This can be combined with
I<optional> or <required>.

=back

Here are some example siglevels:

  $alpm->set_defsiglvl('never') or die $alpm->errstr;
  $alpm->set_defsiglvl('default') or die $alpm->errstr;
  $alpm->set_defsiglvl({ 'pkg' => [ 'optional' ], 'db' => [ 'required', 'trustall' ]}) or die $alpm->errstr;
  $alpm->set_defsiglvl({ 'db' => [ 'optional' ]}) or die $alpm->errstr;

I<TODO: I should probably employ simple strings, split on spaces, instead of arrayrefs in
the off-chance that trustall is used. This would look nicer.>
...
up with an "or die". Something like this:
  ...
  $db->force_update or die $alpm->errstr;

This is annoying but not unlike most other perl error checking. If you find yourself
calling methods on an undefined value then an error probably occurred.
...
Copyright (C) 2012 by Justin Davis
