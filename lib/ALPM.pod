...
=head2 load_pkgfile
	...
	$PKG = $PM->load_pkgfile($FILE, $FULL, $SIGLEVEL);

=over 4

=item C<$PKG> - Package object.

=item C<$PM> - ALPM object.

=item C<$FILE> - Path to a package file (i.e. pkg.tar.xz).

=item C<$FULL> - Full (1) or partial load (0). (trust me, don't install partial loads)

=item C<$SIGLEVEL> - Signal level hashref. Keys are type and level.
